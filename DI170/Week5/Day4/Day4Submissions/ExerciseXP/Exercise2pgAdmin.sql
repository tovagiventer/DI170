...
SELECT film_id, title, description, length, rental_rate FROM film WHERE title IN ('Dirty Dancing');
...
SELECT * from film ORDER BY rental_rate ASC, film_id ASC LIMIT 10;

-- Not satisfied with the results. Write a query which will find the next 10 cheapest movies.
-- Bonus: Try to not use LIMIT.

SELECT * from film ORDER BY rental_rate ASC, film_id ASC LIMIT 10 OFFSET 10;
...
SELECT customer.first_name, customer.last_name, payment.amount, payment.payment_date FROM customer INNER JOIN payment ON customer.customer_id = payment.customer_id ORDER BY payment.customer_id ASC;
...
SELECT customer.customer_id, customer.first_name, customer.last_name, payment.amount, payment.payment_date FROM payment INNER JOIN customer ON customer.customer_id = payment.customer_id ORDER BY staff_id ASC;
